Module HartreeFock
    ...
    end subroutine Convert_AO2MO_G2
			...
			do q=1,DK 
				do r=1,DK 
					do s=1,DK 
						...
						value1 = G2_MO((p+1)/2,(r+1)/2,(q+1)/2,(s+1)/2) * spins(1)*spins(2)
						value2 = G2_MO((p+1)/2,(r+1)/2,(q+1)/2,(s+1)/2) * spins(3)*spins(4)
					    G_SMO(p,q,r,s)=value1-value2
					end do 
				end do
			end do 
		end do 
    end subroutine Joshua
    ...
    subroutine Get_EMP2
		...
		do i=1,N
			do j=1,N
				do a=N+1,DK
					do b=N+1,DK
						EMP2=EMP2+0.25*G_SMO(i,j,a,b)**2/&
						!&(MOEnergy(i)+MOEnergy(j)-MOEnergy(a)-MOEnergy(b))
						&(fs(i)+fs(j)-fs(a)-fs(b))
					end do 
				end do
			end do 
